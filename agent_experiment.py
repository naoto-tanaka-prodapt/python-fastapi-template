...
def run_orchestrator_agent(session_id, job_id):
    ...
    while user_reply != "bye":
        orch_prompt = ChatPromptTemplate.from_messages(messages)
        orch_chain = orch_prompt | model
        orch_output = orch_chain.invoke({"session_id": session_id, "job_id": job_id})

        data = json.loads(orch_output.content)
        print(data)
        messages.append(("assistant", orch_output.content.replace("{", "{{").replace("}", "}}")))
        ...
        if "tool_name" in data and data["tool_name"] != "":
            ...
            args = { item["param"]: item["value"] for item in data["tool_params"] }
            response = tool_function(**args)            
            messages.append(("ai", str(response)))
        else:
